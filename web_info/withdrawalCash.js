;(function () {
	...
	var $coin = $content.find(".userCoin");
	var $withdrawCoin = $content.find(".userWithdrawalCoin");
	...
	PAGE.ajax({
		...
		url: "/api/homeland/coins?token=" + token,
		success: function (ret) {
			if (ret) {
				...
				// 	"平安银行":10
				// }
				$coin.html(ret.number);
				$withdrawCoin.html(ret.deposit);
				var $dialogContent = $("#withdrawlDialog")
				$dialogContent.find(".userWithdrawalCoin").html(ret.deposit);
				...
				if (ret.log) {
					for (var i = 0; i < ret.log.length; i++) {
						...
						var map = {"0": {name:"摇钱树收获",value:1},
						 "1":{name:"偷取朋友摇钱树",value:1} ,
						 "2":{name:"朋友消费",value:1} , 
						 "3": {name:"金币提现",value:-1}}
	...
	function withDrawDialog() {
		...
		var validForm = PAGE.validForm({
			validSuccess: function ($form) {
				...
				PAGE.ajax({
					data: $form.serialize() + "&token=" + token+"&type=1",
					type: 'get',
					msg: {
						"0": "登录token验证失败",
						"1": " 申请成功",
