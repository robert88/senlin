...
layui.define(function(exports){
  ...
  Page.prototype.view = function(){
    ...
    //当前页非首页，则输出上一页
      if(conf.prev){
          if(conf.showPervAway){
              if(conf.curr == 1 ){
                  view.push('<a href="javascript:;" class="layui-laypage-prev layui-laypage-prev-muted">'+ conf.prev +'</a>');
              }else{
    ...
    if(conf.curr !== conf.pages && conf.next || dict.flow){
      ...
      if(conf.showPervAway){
          view.push('<a href="javascript:;" class="layui-laypage-next layui-laypage-prev-muted" data-page="'+ (conf.curr ) +'">'+ conf.next +'</a>')
      }
    }
      if(conf.showTotal){
          view.push('<span href="javascript:;" class="layui-laypage-total">共<em class="layui-laypage-total-num">'+conf.total+'</em>条记录</span>')
      }
